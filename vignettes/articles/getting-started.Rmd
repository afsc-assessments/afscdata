...
Step 0.5. Setup your `keyring` to store passwords and user names. 
...
Load the library and connect to both AKFIN and AFSC servers.
...
First we will setup a file structure to work with the `afscdata` package. 
...
With a folder structure in place we now can directly save files, as opposed to bringing them into the global environment then saving them.

Let's retrieve bottom trawl survey length data from the GOA for northern rockfish. 
...
```{r basic}
q_bts_length(year=year, species=30420, area='goa', db=afsc)
```
